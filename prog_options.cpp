...
static opts::options_description sensor_options_desc(prog_options& options) {
  ...
  ("samplerate,s",
   opts::value<int>(&options.samplerate)->default_value(50),
   "Samplerate for sensor data");
  return desc;
}

static opts::options_description processing_options_desc(
    prog_options& options) {
  opts::options_description desc("Processing options");
  desc.add_options()(
      "mode", opts::value<std::string>(&options.mode)->default_value("normal"),
...
static opts::options_description command_line_options_desc(
    prog_options& options) {
...
static opts::options_description config_file_options_desc(
    prog_options& options) {
